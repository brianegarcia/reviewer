...
const CTX = "phase4/lab-result-documents";
const S3_PREFIX = "imports/practice-fusion/lab-results";
const UPLOAD_CONCURRENCY = 10;

/**
 * Uploads lab result PDFs to S3 and creates `lab_order_files` records.
 * TSV columns: PatientPracticeGuid, ResultGuid, OrderGuid, DocumentStorageGuid, LastModifiedDateTimeUtc
 */
export async function importLabResultDocuments(labOrderMap: IdMap): Promise<void> {
    logger.info(CTX, "Starting lab result document import...");

    const sequelize = getSequelize();
    const rows = await parseTsvFile(TSV_FILES.labResultDocuments);
    logger.info(CTX, `Found ${rows.length} lab result document records`);
            ...
            await sequelize.query(
                 ...
                 VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())`,
                {
                    bind: [
                        ...
                        "application/pdf",
                        size,
                        "Practice Fusion lab result import",
                    ],
                }
            );
            imported++;
